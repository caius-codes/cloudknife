...
def search_modules(session_mgr, query: Optional[str]) -> None:
    """
    Search Azure modules by keyword.
    ...
    Args:
        session_mgr: Azure session manager instance (unused but required for CLI consistency)
    ...
    if not query:
        ...
        console.print("[dim]Example: search user[/dim]")
        return

    if _registry is None:
        console.print("[dim]Building module index...[/dim]")
        try:
            _registry = ModuleRegistry.discover("azure")
    ...
    if not results:
        ...
        console.print("[dim]Try: 'user', 'storage', 'mfa', 'teams', 'enumerate'[/dim]")
        return

    table = Table(title=f"Azure Modules matching '{query}' ({len(results)} found)")
