...
def search_modules(session_mgr, query: Optional[str]) -> None:
    ...
    Args:
        session_mgr: AWS session manager instance (unused but required for CLI consistency)
    ...
    if not query:
        ...
        console.print("[dim]Example: search s3[/dim]")
        return

    if _registry is None:
        console.print("[dim]Building module index...[/dim]")
        try:
            _registry = ModuleRegistry.discover("aws")
    ...
    if not results:
        ...
        console.print("[dim]Try: 'iam', 's3', 'lambda', 'privilege', 'enumerate'[/dim]")
        return

    table = Table(title=f"AWS Modules matching '{query}' ({len(results)} found)")
